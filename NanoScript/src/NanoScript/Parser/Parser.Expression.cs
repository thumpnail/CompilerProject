...
public partial class Parser {
    ...
    private IExpression? ParseFactorExpression() {
        ...
        if ((exp = ParseUnaryExpression()) is not null) {
    ...
    private IExpression? ParseExpression() {
        ...
        IExpression? res = null;
        IExpression? exp;
        var type = GuessExpressionType();
        //[ ... ]
        if (type.Equals(ExpressionType.arraydecl)) {
            ctx.Consume_tk(LEFTBRACKET);
            ...
            ctx.Consume_tk(RIGHTBRACKET); 
            //abc{...}
        } else if (type.Equals(ExpressionType.objectdecl)) {
	        var ident = ParseIdentifierExpression();
	        ...
	        //fnc ( ... ) : abc { ... }
        } else if (type.Equals(ExpressionType.functiondecl)) {
	        FunctionDeclarationExpression function_expression = new();
	        //function expression
	        ctx.Consume_tk(FNC);
	        ctx.Consume_tk(LEFTPAREN);
	        //Parse the parameter expression
	        function_expression.parameters = ParseParameterDeclarationList() ?? new();
	        ctx.Consume_tk(RIGHTPAREN);
	        function_expression.typeDeclarationStatement = ParseTypeDeclarationStatement();
	        ctx.Consume_tk(LEFTBRACE);
	        //parse statements
	        function_expression.statements = ParseStatements();
	        ctx.Consume_tk(RIGHTBRACE);
	        res = function_expression;
    ...
    private ExpressionType GuessExpressionType() {
	    var res = ExpressionType.none;
	    if (ctx.Peek_tk(IDENTIFIER,NUMBER,STRING,TRUE,FALSE)) {
		    res = ExpressionType.valuedecl;
	    } else if (ctx.Peek_tk(FNC, LEFTBRACE)) {
		    res = ExpressionType.functiondecl;
	    } else if (ctx.Peek_tk(LEFTBRACKET) && ctx.PeekNext_tk(IDENTIFIER,NUMBER,STRING,TRUE,FALSE)) {
		    res = ExpressionType.arraydecl;
	    }
	    return res;
    }
    //<list> := <exp> {',' <exp>}
