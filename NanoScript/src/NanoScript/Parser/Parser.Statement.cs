...
public partial class Parser {
    ...
    private List<IStatement> ParseStatements(Token delimiter = NONE) {
        var res = new List<IStatement>();
        if (delimiter == NONE) {
            IStatement tmp;
            while ((tmp = ParseStatement()) is not null) {
                res.Add(tmp);
            }
        } else {
            IStatement tmp;
            while (!ctx.Peek_tk(delimiter) && (tmp = ParseStatement()) is not null) {
                res.Add(tmp);
            }
        }
        return res;
    ...
    private List<ImportStatement> ParseImportStatements() {
        ...
        ImportStatement tmp;
        while ((tmp = ParseImportStatement()) is not null) {
            res.Add(tmp);
        }
        return res;
    }
    ...
    private ImportStatement? ParseImportStatement() {
        ...
        if (ctx.Peek_tk(Token.IMPORT) && ctx.PeekNext_tk(Token.STRING)) {
	        ctx.Consume_tk(Token.IMPORT);
	        //todo: parse the given file and add to lexer result
	        res.importString = ctx.Consume() ?? "<|Error parsing import|>";

	        if (!ctx.importedFiles.Contains(res.importString)) {
		        ctx.importedFiles.Add(res.importString);
		        ctx.lexerResult.AddResult(NanoScript.CreateLexer().LexFile(NanoScript.CWD + "\\" + res.importString.Replace("\"","").Replace("\'","")));
	        }
            ...
            ctx.PopFrame("Expected 'import' keyword");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private VariableDeclarationStatement? ParseVariableDeclarationStatement() {
        ...
        if (ctx.Peek_tk(Token.LET) || ctx.Peek_tk(Token.VAR) || ctx.Peek_tk(Token.CONST)) {
            ...
            if (ctx.Peek_tk(Token.EQUAL)) {
                res.isAssign = ctx.Consume_tk(Token.EQUAL);
                res.exp = ParseFullExpression();
            }
        } else {
            ctx.PopFrame("expected 'let'/'var'/'const' kewords for varible declaration");
            return null;
        }
    ...
    private StructDeclarationStatement? ParseStructDeclarationStatement() {
        ...
        if (ctx.Consume_tk(STRUCT)) {
            ...
            ctx.PopFrame("expected 'struct' keyword");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private ClassDeclarationStatement? ParseClassDeclarationStatement() {
        ...
        if (ctx.Consume_tk(CLASS)) {
            ...
            ctx.PopFrame("expected 'class' keyword");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private FunctionDeclarationStatement? ParseFunctionDeclarationStatement() {
        ...
        if (ctx.Consume_tk(FNC)) {
            ...
            ctx.PopFrame("expected 'fnc' keyword");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private List<ParameterDeclaration>? ParseParameterDeclarationList() {
        ...
        if (ctx.Peek_tk(IDENTIFIER)) {
            ...
            ctx.PopFrame("expected identifier");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private FunctionCallStatement? ParseFunctionCallStatement() {
        ...
        if (ctx.Peek_tk(IDENTIFIER)) {
            ...
            ctx.PopFrame("expected function call statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private ReturnStatement? ParseReturnStatement() {
        ...
        if (ctx.Peek_tk(RETURN)) {
            ...
            ctx.PopFrame("expected return statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private EnumDeclarationStatement? ParseEnumDeclarationStatement() {
        ...
        if (ctx.Peek_tk(ENUM)) {
            ...
            ctx.PopFrame("expected enum declaration");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private EnumValueDeclaration? ParseEnumValueDeclaration() {
        ...
        if (ctx.Peek_tk(IDENTIFIER)) {
            ...
            ctx.PopFrame("expected enum value declaration");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private AssignmentStatement? ParseAssignmentStatement() {
        ...
        if (ctx.Peek_tk(IDENTIFIER)) {
            ...
            ctx.PopFrame("expected assignment statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private ConditionalStatement? ParseConditionalStatement() {
        ...
        if (ctx.Peek_tk(IF)) {
            ...
            ctx.PopFrame("expected conditional statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private SwitchStatement? ParseSwitchStatement() {
        ...
        if (ctx.Peek_tk(SWITCH)) {
            ...
            ctx.PopFrame("expected switch statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private SubSwitchStatement? ParseSubSwitchStatement() {
        ...
        if (ctx.Peek_tk(IDENTIFIER)) {
            ...
            ctx.PopFrame("expected sub-switch statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private ForStatement? ParseForStatement() {
        ...
        if (ctx.Peek_tk(FOR)) {
            ...
            ctx.PopFrame("expected for statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private ErrorStatement? ParseErrorStatement() {
        ...
        if (ctx.Peek_tk(ERROR)) {
            ...
            ctx.PopFrame("expected error statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private BreakContinueStatement? ParseBreakContinueStatement() {
        ...
        if (ctx.Peek_tk(BREAK) || ctx.Peek_tk(CONTINUE)) {
            ...
            ctx.PopFrame("expected break or continue statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private InterfaceStatement? ParseInterfaceStatement() {
        ...
        if (ctx.Peek_tk(INTERFACE)) {
            ...
            ctx.PopFrame("expected interface statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private DeclarationStatement? ParseDeclarationStatement() {
        ...
        if (ctx.Peek_tk(DEF) || ctx.Peek_tk(TYPE)) {
            ...
            ctx.PopFrame("expected declaration statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private AssertionStatement? ParseAssertionStatement() {
        ...
        if (ctx.Peek_tk(ASSERT)) {
            ...
            ctx.PopFrame("expected assertion statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private LabelStatement? ParseLabelStatement() {
        ...
        if (ctx.Peek_tk(DOUBLECOLON)) {
            ...
            ctx.PopFrame("expected label statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private GotoStatement? ParseGotoStatement() {
        ...
        if (ctx.Peek_tk(GOTO)) {
            ...
            ctx.PopFrame("expected goto statement");
            return null;
        }
        ctx.ClearFrame();
        return res;
    ...
    private TypeDeclarationStatement? ParseTypeDeclarationStatement() {
        ...
        if (ctx.Consume_tk(Token.COLON)) {
            if (ctx.Consume_tk(Token.LEFTBRACKET)) {
                res.typeDeclarationType = TypeDeclarationType.array;
                if (!ctx.Peek_tk(Token.RIGHTBRACKET))
                    res.exp = ParseFullExpression();
                ctx.Consume_tk(Token.RIGHTBRACKET);
                res.identifier = ParseIdentifierExpression();
            } else if (ctx.Consume_tk(Token.LEFTBRACE)) {
                res.typeDeclarationType = TypeDeclarationType.table;
                ctx.Consume_tk(Token.RIGHTBRACE);
            } else if (ctx.Consume_tk(Token.DOUBLEBRACES)) {
                res.typeDeclarationType = TypeDeclarationType.table;
            } else {
            ...
            ctx.PopFrame("expected type declaration");
            return null;
        }
        ctx.ClearFrame();
        return res;
