...
public class ParserContext {
    ...
    //TODO: Convert all to throwing functions and add 'try' function that dont throw
    ...
    public bool PeekNext_tk(params Token[] token) {
        if (boundCheck(idx+1)) {
	        foreach (var item in token) {
		        if (item == tokens[idx+1].token) return true;
	        }
        }
